...
, ICloneable
#endif
    {
        ...
        /// </summary>
        /// <remarks>This method allows out-of-bound (blocked) positions.</remarks>
        public Rectangle GetBaseLayerSourceRectangleMinusOne(Point mapPosition)
        {
            // check the parameter, but out-of-bounds is nonfatal
            if ((mapPosition.X < 0) || (mapPosition.X >= mapDimensions.X) ||
                (mapPosition.Y < 0) || (mapPosition.Y >= mapDimensions.Y))
            ...
            int baseLayerValue = GetBaseLayerValue(mapPosition)-1;
            if (baseLayerValue < 0)
            {
                return Rectangle.Empty;
            }
        ...
        /// </summary>
        /// <remarks>This method allows out-of-bound (blocked) positions.</remarks>
        public Rectangle GetFringeLayerSourceRectangleMinusOne(Point mapPosition)
        {
            // check the parameter, but out-of-bounds is nonfatal
            if ((mapPosition.X < 0) || (mapPosition.X >= mapDimensions.X) ||
                (mapPosition.Y < 0) || (mapPosition.Y >= mapDimensions.Y))
            ...
            int fringeLayerValue = GetFringeLayerValue(mapPosition)-1;
            if (fringeLayerValue < 0)
            {
                return Rectangle.Empty;
            }
        ...
        /// </summary>
        /// <remarks>This method allows out-of-bound (blocked) positions.</remarks>
        public Rectangle GetObjectLayerSourceRectangleMinusOne(Point mapPosition)
        {
            // check the parameter, but out-of-bounds is nonfatal
            if ((mapPosition.X < 0) || (mapPosition.X >= mapDimensions.X) ||
                (mapPosition.Y < 0) || (mapPosition.Y >= mapDimensions.Y))
            ...
            int objectLayerValue = GetObjectLayerValue(mapPosition)-1;
            if (objectLayerValue < 0)
            {
                return Rectangle.Empty;
            }
        ...
        /// <summary>
        /// The content names, positions, and orientations of the Blocks on this map.
        /// </summary>
        private List<MapEntry<Block>> blockEntries =
            new List<MapEntry<Block>>();

        /// <summary>
        /// The content names, positions, and orientations of the Blocks on this map.
        /// </summary>
        public List<MapEntry<Block>> BlockEntries
        {
            get { return blockEntries; }
            set { blockEntries = value; }
        }


        /// <summary>
        /// The content names, positions, and orientations of the Switchs on this map.
        /// </summary>
        private List<MapEntry<Switch>> switchEntries =
            new List<MapEntry<Switch>>();

        /// <summary>
        /// The content names, positions, and orientations of the Switchs on this map.
        /// </summary>
        public List<MapEntry<Switch>> SwitchEntries
        {
            get { return switchEntries; }
            set { switchEntries = value; }
        }
        ...
        public object Clone()
        {
            ...
            map.blockEntries.AddRange(BlockEntries);
            map.switchEntries.AddRange(SwitchEntries);
            map.texture = Texture;
        ...
        public class MapReader : ContentTypeReader<Map>
        {
            ...
            protected override Map Read(ContentReader input, Map existingInstance)
            {
                ...
                map.BlockEntries.AddRange(input.ReadObject<List<MapEntry<Block>>>());
                foreach (MapEntry<Block> blockEntry in map.BlockEntries)
                {
                    blockEntry.Content = input.ContentManager.Load<Block>(
                        System.IO.Path.Combine(@"Maps\Blocks",
                        blockEntry.ContentName)).Clone() as Block;
                }

                map.SwitchEntries.AddRange(input.ReadObject<List<MapEntry<Switch>>>());
                foreach (MapEntry<Switch> switchEntry in map.SwitchEntries)
                {
                    switchEntry.Content = input.ContentManager.Load<Switch>(
